...
const AddNewFriend: FC<IProps> = memo(({ modal, setModal }) => {
	...
	const errorMessages = useAppSelector((state) => state.chatSlice.errorsMessages);
	...
	const handleSubmit = (values: IValues, help: FormikHelpers<IValues>) => {
		if (handleValidation(values)) {
			...
			dispatch(chatActions.setNewFriend(values.username.toLowerCase()));
		}
	...
	const wrapperClassList =
		(modal ? 'transition-all ease-in-out duration-500 visible opacity-100' : 'invisible opacity-0') +
		' transition-all ease-in-out duration-500 absolute left-0 top-0 z-10 w-full h-screen backdrop-blur-sm';
	const classList =
		(modal ? 'transition-all duration-500 visible ease-in-out opacity-100' : 'invisible opacity-0') +
	...
	return (
		<>
			<div className={classList}>
				<h3 className='text-white text-2xl text-center mb-6 font-semibold'>Add friend</h3>
				...
				<Formik
					...
					<Form className='flex flex-col text-green-500 font-bold w-full h-full'>
						...
						<Field className='text-black rounded-xl px-3 mt-1 py-2' id='username' name='username' placeholder='John' />
